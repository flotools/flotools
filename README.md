...
Flo Tools uses 2 distinct sounds to provide auditory feedback. The first is [This Sound](http://file:///System/Library/Sounds/Pop.aiff), which signals that something is in process and the user needs to wait for Flo Tools to complete a task. An example of this would be deselecting all tracks in a large session.

The second is [This Sound](http://file:///System/Library/Sounds/Tink.aiff), which tells a user that something has been toggled, such as solo on and off.
